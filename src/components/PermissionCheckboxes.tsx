...
export default function PermissionCheckboxes({
      ...
      if (data.success) {
        ...
        if (shouldAutoFill) {
          ...
          if (isRoleChanged && selectedPermissions.length > 0) {
            const confirmed = await showConfirm(`Change permissions to ${roleValue} template? Your current selections will be replaced.`);
            if (confirmed) {
  ...
  const handleResetToTemplate = async () => {
    const confirmed = await showConfirm('Reset permissions to role template?');
    if (confirmed) {
  ...
  const handleClearAll = async () => {
    const confirmed = await showConfirm('Clear all permissions?');
    if (confirmed) {
      onChange([]);
    }
  };
