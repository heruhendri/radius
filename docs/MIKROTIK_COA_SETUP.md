...
Cek di aplikasi web: **Admin → Network → Routers** → pilih router → lihat field "Secret"
...
2. **Framed-IP tidak match**
   ...
   - Cek di Mikrotik: `/ip hotspot active print` → kolom "address"
...
Di web admin:
- Admin → Hotspot → Voucher → Generate
...
[CoA] ✓ Successfully sent disconnect for XXXXXX
[CRON] ✓ Disconnected XXXXXX from test (172.20.30.11)
```

### 4. Verifikasi user ter-disconnect

- Cek di Mikrotik: `/ip hotspot active print` → voucher sudah hilang
- Cek di web: Admin → Sessions → Hotspot → voucher sudah hilang
