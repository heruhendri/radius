...
```
src/lib/cron/
├── config.ts           # Job definitions & schedules
├── voucher-sync.ts     # Voucher & session jobs
├── agent.ts            # Agent sales jobs
├── isolir.ts           # Auto isolir jobs
├── invoice.ts          # Invoice generation jobs
└── whatsapp.ts         # WhatsApp queue jobs
