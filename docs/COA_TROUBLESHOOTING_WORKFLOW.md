...
### Status Saat Ini
- ✅ Network connectivity: VPS dapat ping dan reach port 3799 UDP Mikrotik
- ✅ CoA enabled di Mikrotik: `/radius incoming accept=yes port=3799`
- ✅ Secret benar: `secret123`
- ❌ **Mikrotik tidak merespon CoA request:** `No reply from server`
...
1. **Secret di RADIUS Incoming berbeda dengan RADIUS Client**
   - Mikrotik punya 2 tempat konfigurasi RADIUS dengan secret berbeda:
     - `/radius` → secret untuk **outgoing** (Mikrotik → FreeRADIUS)
     - `/radius incoming` → secret untuk **incoming** (FreeRADIUS → Mikrotik)
...
## 🔧 SOLUSI ALTERNATIF (Jika CoA Tetap Gagal)
...
if (activeSession) {
  ...
  console.log(`[CRON] ✓ Marked session ${voucher.code} as stopped in database`)
}
```
...
1. ✅ Set `default-secret` di `/radius incoming` Mikrotik
2. ✅ Restart Mikrotik
3. ✅ Test dengan `/root/test-coa.sh`
4. ✅ Jika sukses, CoA ready!
...
- ❌ Overhead untuk cron job (setiap menit)
- ❌ Butuh RouterOS API library
- ❌ Connection pool management kompleks
- ❌ Security risk (harus expose API port)

**Gunakan FreeRADIUS standard CoA** karena:
- ✅ Lightweight (UDP packet)
- ✅ RFC 5176 standard
- ✅ No extra dependencies
- ✅ Supported semua Mikrotik modern
- ✅ Built-in di FreeRADIUS
