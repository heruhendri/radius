...
Ini untuk **Mikrotik → FreeRADIUS** (authentication):
...
Ini untuk **FreeRADIUS → Mikrotik** (CoA disconnect):
...
5. **Verify user disconnect:**
   - Di Mikrotik: `/ip hotspot active print` → voucher hilang
   - Di web: Admin → Sessions → Hotspot → voucher hilang
...
- [x] Network connectivity VPS ↔ Mikrotik OK
...
- `/radius print stats` → cari bad requests
- `/ip hotspot active print` → pastikan expired user disconnect
